...
function scoreEcommerceOffer(ctx, max) {
  ...
  const productStrength = strengthFromCount(meta.high_confidence_product_count || 0, {
    weak: 1,
    medium: 2,
    strong: 3,
  })
  addSignal(result, {
    ...
    maxPoints: 6,
    label: `${meta.high_confidence_product_count || 0} high-confidence product(s) with clear names.`,
    evidence: `Product cards/detail pages: ${meta.has_reliable_product_cards ? 'yes' : 'no'}, JSON-LD: ${meta.has_json_ld_products ? 'yes' : 'no'}.`,
    problem: productStrength === 'none' ? 'No reliable product cards or product pages were found.' : null,
  ...
  if ((aggregated.products || []).length === 0) {
    ...
    result.problems.push('Ecommerce store has no extractable products — offer clarity is severely limited.')
  ...
  if (!meta.has_reliable_product_cards && (aggregated.products || []).length > 0) {
    result.earned = Math.min(result.earned, Math.round(max * 0.55))
...
function scorePhysicalServiceOffer(ctx, max) {
  ...
  const serviceStrength = strengthFromBoolean(signals.has_service_categories)
  addSignal(result, {
    id: 'services',
    strength: serviceStrength,
    points: pointsForStrength(serviceStrength, 5),
    maxPoints: 5,
    label: 'Dedicated service pages or categories explain what you do.',
    evidence: `${aggregated.services?.length || 0} service page(s) crawled.`,
    problem: serviceStrength === 'none' ? 'No clear service pages or categories found.' : null,
    fix: 'Create service pages with scope, pricing hints, and outcomes.',
  })

  const bookingStrength = combineStrengths([
    strengthFromBoolean(signals.has_booking_cta),
    strengthFromBoolean(signals.has_quote_cta),
    strengthFromBoolean(signals.has_consultation),
  ])
  addSignal(result, {
    id: 'booking',
    strength: bookingStrength,
    points: pointsForStrength(bookingStrength, 5),
    maxPoints: 5,
    label: 'Quote, booking, or consultation CTA matches service sales flow.',
    evidence: `Booking=${signals.has_booking_cta}, quote=${signals.has_quote_cta}.`,
    problem: bookingStrength === 'none' ? 'No quote, booking, or consultation CTA detected.' : null,
    fix: 'Add Book now or Get a free quote above the fold.',
  })

  const areaStrength = combineStrengths([
    strengthFromBoolean(signals.has_service_area),
    strengthFromBoolean(signals.has_local_city),
  ])
  addSignal(result, {
    id: 'service_area',
  ...
  const galleryStrength = strengthFromBoolean(signals.has_gallery)
  addSignal(result, {
    id: 'portfolio',
  ...
  const contactStrength = combineStrengths([
    strengthFromBoolean(signals.has_phone),
    strengthFromBoolean(signals.has_contact_page),
  ])
  addSignal(result, {
    id: 'contact',
    ...
    evidence: `Phone=${signals.has_phone}, contact page=${signals.has_contact_page}.`,
...
function scoreGalleryPhysicalServiceOffer(ctx, max) {
  ...
  const galleryStrength = combineStrengths([
    strengthFromBoolean(signals.has_gallery),
  ...
  addSignal(result, {
    ...
    maxPoints: 7,
    label: 'Gallery or portfolio showcases work quality.',
    evidence: evidenceLabel(galleryStrength),
    problem: galleryStrength === 'none' ? 'No gallery or portfolio proof detected.' : null,
    fix: 'Add a visual gallery with project photos and short captions.',
  })
  ...
  addSignal(result, {
    ...
    maxPoints: 6,
    label: 'Consultation, quote, or contact path supports offline sales.',
    evidence: `Quote=${signals.has_quote_cta}, contact=${signals.has_contact_page}, phone=${signals.has_phone}.`,
  ...
  const serviceStrength = strengthFromBoolean(signals.has_service_pages || signals.has_service_area)
  addSignal(result, {
    id: 'service_explanation',
    strength: serviceStrength,
    points: pointsForStrength(serviceStrength, 4),
    maxPoints: 4,
    label: 'Service explanation helps visitors understand what you deliver.',
    evidence: evidenceLabel(serviceStrength),
    problem: serviceStrength === 'none' ? 'Service explanation is thin or missing.' : null,
    fix: 'Describe services, process, and outcomes on dedicated pages.',
  })
